...
class CrudeOilPredictsEqeuityReturns(QCAlgorithm):
    ...
    def Total_Risk_Contribution_errors(self,Weights,args):
        ...
        RC_taken = np.square(Weights*Margin_cs/self.Lens - budgetting_weights*portfolio_risk).sum()
        return RC_taken
    ...
    def _get_risk_parity_weights(self,init_Weights,X,Monthly_History_X): #Optimisation fuction
        ...
        constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x)}) #Add -1 for Long only portfolios
                       
                       
        bnds = tuple((-0.10,np.maximum(0.10,init_Weights[0]+0.01)) for s in init_Weights) #Replace left array by 1% for Long only Portfolio
